...
public final class CryptoUtilities {
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        powerMod(wtemp1, two, ntemp1);
        if (wtemp1.compareTo(one) == 0) {
            witness = true;
        }

        powerMod(wtemp2, ntemp2, ntemp1);
        if (wtemp2.compareTo(one) == 0) {
            witness = false;
        }
    ...
    public static boolean isPrime1(NaturalNumber n) {
        ...
        if (n.compareTo(three) <= 0) {
            ...
            prime = !isWitnessToCompositeness(two, n);
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        int i = 1;
        while (prime == true && i < 20) {
            NaturalNumber copy = new NaturalNumber2(n);
            copy.decrement();

            NaturalNumber value = n.newInstance();
            value.increment();

            NaturalNumber witness = randomNumber(n);

            while (!(witness.compareTo(value) > 0
                    && witness.compareTo(copy) < 0)) {
